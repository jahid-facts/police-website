...
export default function WorkCategory() {
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    http.delete(`delete-work-document-section/${id}`)
                        .then((res) => {
                            setUpdate(!update);
                            setIsOpen(false);
                            Swal.fire({
    ...
    const editData = (id) => {
        http.get(`work-document-section/${id}`)
            .then((res) => {
    ...
    const submitData = (e) => {
        ...
        if (categoryData.id) {
            http.put(`update-work-document-section/${categoryData.id}`, formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
            ...
            http.post('save-work-document-section', formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
                    setCategoryData({ image: "", title: "", });
                    ...
                    <div className="page-content">
                        ...
                        <div className=" p-3">
                            <form onSubmit={submitData}>
                                ...
                                {
                                    categoryData.id ?
                                        <input className="form-control form-control-sm my-2" name="image" onChange={(e) => setCategoryData({ ...categoryData, image: e.target.files[0] })} type="file" accept='image/*' />
                                        :
                                        <input required className="form-control form-control-sm my-2" name="image" onChange={(e) => setCategoryData({ ...categoryData, image: e.target.files[0] })} type="file" accept='image/*' />
                                }
