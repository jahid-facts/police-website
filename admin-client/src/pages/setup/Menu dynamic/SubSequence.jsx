...
export default function BitPolicingNews() {
    ...
    const deleteRowData = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                http.delete(`delete-sub-sequence/${id}`)
                    .then((res) => {
                        setUpdate(!update);
                        setIsOpen(false);
                        Swal.fire({
            ...
            render: (row) => (
                <div>
                    {administration.includes('Edit Menu Section') && (
                        <button onClick={() => editData(row.id)} className='btn btn-sm action-btn'>
                            <i className='far fa-edit'></i>
                        </button>
                    )}
                    {administration.includes('Delete Menu Section') && (
                        <button onClick={() => deleteRowData(row.id)} className='btn btn-sm action-btn'>
    ...
    const submitData = (e) => {
        ...
        formData.append('subpagesId', categoryData.officerId);

        if (categoryData.id) {
            http.put(`update-sub-sequence/${categoryData.id}`, formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
                    setPagesData({ title: '', subpagesId: '' });
                    setContent('');
                    Swal.fire({
                        position: 'top-center',
                        icon: 'success',
            ...
            http.post('create-sub-sequence', formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
                    setContent('');
        ...
        <div className='page-content adjustment-type-table'>
            <div className='custom-card p-2 d-flex justify-content-between mb-2 align-items-center'>
                ...
                {administration.includes('Add Menu Section') && (
                    <div>
                        <button style={{ marginTop: '1px' }} onClick={addPage} className='btn btn-sm btn-primary float-end'>
                            Add
                        </button>
                    </div>
                )}
                    ...
                    <div className='page-content'>
                        ...
                        <div className=' p-3'>
                            <form onSubmit={submitData}>
                                ...
                                {categoryData.id ? (
                                    <input
                                        onChange={(e) => setPagesData({ ...categoryData, image: e.target.files[0] })}
                                        type='file'
                                        className='form-control form-control-sm my-2'
                                        placeholder='Title'
                                    />
...
// export default function SubSequence({ subpagesId }) {
//     const userRole = useContext(RoleContext);
//     const { administration } = userRole;
...
//         e.preventDefault();
//         const formData = {
//             // subpagesId: Number(categoryData.subpagesId),
//             subpagesId: user.permissions,
...
//                     <div className="page-content">
//                         <div className=" patients-head ">
//                             <h5 className="fw-normal px-2  text-start card-name">Add Page
...
//                                         <input onChange={handleChange} value={categoryData.index} name="index" type="text" className="form-control form-control-sm my-2" required placeholder="Index" /> */}
...
//                                         <select name="status" onChange={handleChange} value={categoryData.status} className="form-control form-control-sm my-2" required>
//                                             <option value="Active">Active</option>
//                                             <option value="Inactive">Inactive</option>
//                                         </select>
//                                         <label htmlFor="content">Content</label>
//                                         <textarea onChange={handleChange} value={categoryData.content} name="content" type="text" className="form-control form-control-sm my-2" required placeholder="Content" />
//                                         <button className="btn mt-4  btn-sm btn-success float-end text-uppercase" type="submit">
//                                             <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-save mb-1">
//                                                 <path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path>
//                                                 <polyline points="17 21 17 13 7 13 7 21"></polyline>
//                                                 <polyline points="7 3 7 8 15 8"></polyline>
